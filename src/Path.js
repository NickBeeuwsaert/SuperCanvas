...
Path.each = function each(path, fn=segment=>segment, thisArg=null) {
    if (typeof path === "string") {
        path = Path.parsePath(path);
    }
...
function convertToBeziers(path, x, y) {
    ...
    switch(command) {
        case 'L':
            return ['C', ...lineToCubicCurve(x, y, ...path).slice(2)];
        case 'Q':
            return ['C', ...quadraticCurveToCubicCurve(x, y, ...path).slice(2)];
...
Path.interpolate = function interpolate(path_a, path_b, t) {
    ...
    var interpolatedPoints = zip(
        [].concat(...Path.map(path_a, convertToBeziers).map(extractControlPoints)),
        [].concat(...Path.map(path_b, convertToBeziers).map(extractControlPoints))
    ).map(([cp0, cp1]) => lerp(cp0, cp1, t));

    var result = [
        ['M', ...interpolatedPoints.splice(0, 2)]
    ];

    while(interpolatedPoints.length) {
        result.push(['C', ...interpolatedPoints.splice(0, Path.pathLengths["C"])]);
    }
