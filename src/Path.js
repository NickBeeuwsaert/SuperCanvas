...
* @return Boolean
*/
const commandIsRelative = (command) => command[0].toLowerCase() === command;

const cubicBezierCurve = (P0, P1, P2, P3, t) => (
    Math.pow(1-t, 3) * P0 +
...
const cubicBezierCurveDerivative = (P0, P1, P2, P3, t) => (
    3 * Math.pow(t, 2) * (P3 - 3 * P2 + 3 * P1 - P0) +
    6 * t * (P2 - 2 * P1 + P0 ) + 
    3 * (P1 - P0)
);

const quadraticBezierCurveDerivative = (P0, P1, P2, t) => (
    -2 * P0 * (1-t) + 2 * P1 * (1-2 * t) + 2 * t * P2
);
...
const quadraticBezierCurveZeroes = (P0, P1, P2) => (
    (P0 - P1) / (P0 - 2*P1 + P2)
);

function cubicBezierExtents(P0, P1, P2, P3) {
    let points = [P0, P3],
        t = cubicBezierCurveZeroes(P0, P1, P2, P3);

    if(0 < t[0] && t[0] < 1)
        points.push(cubicBezierCurve(P0, P1, P2, P3, t[0]));
    if(0 < t[1] && t[1] < 1)
        points.push(cubicBezierCurve(P0, P1, P2, P3, t[1]));

    return [Math.min(...points), Math.max(...points)];
}

function quadraticBezierExtents(P0, P1, P2) {
    let points = [P0, P2],
        t = quadraticBezierCurveZeroes(P0, P1, P2);

    if(0 < t && t < 1)
        points.push(quadraticBezierCurve(P0, P1, P2, t));
...
function convertQuadraticCurveToCubic(P0, P1, P2) {
    return [
        P0, lerp(P0, P1, 2/3.),
        lerp(P1, P2, 1/3.), P2
    ];
...
function parsePath(d) {
    ...
    for(i = 0; i < pathArr.length; i++) {
        let segment = pathArr[i],
            ...
            command_upper = command.toUpperCase();
 ...
 */
function ellipticalArcInfo(rx, ry, theta, fA, fS, x0, y0, x1, y1) {
    ...
    static getExtents(path) {
        ...
        Path.each(path, function(segment, x, y) {
            ...
            switch(command) {
                ...
                break;
                case 'Q':
                    h.push(...quadraticBezierExtents(x, segment[0], segment[2]));
                    v.push(...quadraticBezierExtents(y, segment[1], segment[3]));
                break;
                case 'C':
                    h.push(...cubicBezierExtents(x, segment[0], segment[2], segment[4]));
                    v.push(...cubicBezierExtents(y, segment[1], segment[3], segment[5]));
                break;
            }
        });
